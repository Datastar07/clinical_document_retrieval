...
def main() -> None:
    ...
    ap.add_argument(
        "--no-visual",
        action="store_true",
        help="Disable ColQwen visual channel (full pipeline otherwise).",
    )
    args = ap.parse_args()

    root = Path.cwd()
    os.environ["CLINICAL_CONFIG"] = str(Path(args.config))
    os.environ["CLINICAL_API_PROFILE"] = "full"
    os.environ["CLINICAL_API_NO_VISUAL"] = "1" if args.no_visual else "0"
    ...
    print(f"  Reviewer UI: http://{args.host}:{args.port}/")
    print(f"  Swagger UI:  http://{args.host}:{args.port}/docs")
    print(f"  Health:      http://{args.host}:{args.port}/health")
    print(
        f"  profile=full  device={resolved} "
        f"(pref={args.device})  no_visual={args.no_visual}  config={args.config}"
    )
